...
public class ProductCategoryServiceImpl implements ProductCategoryService {
    ...
    @Autowired
    public ProductCategoryServiceImpl(ProductCategoryRepository productCategoryRepository, ProductCategoryMapperDto productCategoryMapperDto, ProductGroupRepository productGroupRepository) {
    ...
    public Collection<ProductCategoryDto> filterCategoryByType(Integer typeId) {
        return productCategoryRepository.filterCategoryByType(typeId).stream().map(productCategory -> productCategoryMapperDto.toDTO(productCategory)).toList();
    }

    @Override
    public Collection<ProductCategoryDto> filterCategoryByGroup(Integer groupId) {
        return productCategoryRepository.filterCategoryByGroup(groupId).stream().map(productCategory -> productCategoryMapperDto.toDTO(productCategory)).toList();
    }
}
